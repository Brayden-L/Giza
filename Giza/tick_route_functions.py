...
def flag_notable_ticks(df_source):
    ...
    df_source.loc[
        df_source["Lead Style"].isin(CLEAN_SEND_FIRST), "Flash/Onsight"
    ] = True
    ...
    df_all_dupes = df_source[
        df_source.duplicated(subset="Route ID", keep=False)
    ]  # First we filter for all duplicate entries.
    df_all_worked = df_all_dupes.groupby("Route ID").filter(
        lambda x: ~x["Lead Style"].isin(CLEAN_SEND_FIRST).any()
    )  # Then we remove all groups which have a lead style of flash or onsight to eliminate group 3.
    df_worked_clean_rponly = df_all_worked[
        df_all_worked.groupby("Route ID")["Lead Style"].apply(
            lambda x: x.isin(CLEAN_SEND_WORKED)
        )
    ]  # fell/hungs and TRs remain, so we take ticks from CLEAN_SEND_WORKED.
    if not df_worked_clean_rponly.empty:
        df_worked_clean_earliest = df_worked_clean_rponly.loc[
            df_worked_clean_rponly.groupby("Route ID")["Date"].idxmin()
        ]  # Use only the earliest redpoint to correctly identify the first redpoint.
        df_source.loc[df_worked_clean_earliest.index.values, "Worked Clean"] = True

    # Flag grade breakthrough ticks
    dfbreakthr = df_source[
        (df_source["Flash/Onsight"] == True) | (df_source["Worked Clean"] == True)
    ]
    breakthrough_indexes = (
        dfbreakthr.groupby("Rating", observed=True)["Date"].idxmin().values
    )
    ...
    df_worked_clean = df_all_worked.groupby("Route ID").filter(
        lambda x: x["Lead Style"].isin(CLEAN_SEND_WORKED).any()
    )  # Filters out worked climbs that were never done clean.
    if not df_worked_clean.empty:
        num_to_send = df_worked_clean.groupby("Route ID").apply(
            lambda x: count_attempt2rp(x, x.iloc[0]["Pitches"])
        )
        num_to_send.rename("Attempts", inplace=True)
        matched_attempts = df_source[df_source["Worked Clean"] == True].merge(
            num_to_send, left_on="Route ID", right_index=True, how="left"
        )
        matched_attempts.index = df_source[
            df_source["Worked Clean"] == True
        ].index  # I'm dumb and this is the best way I could find to get my index to remain
        df_source.loc[matched_attempts.index, "Attempts"] = matched_attempts.iloc[:, -1]
        df_source.loc[
            df_source["Attempts"] == 1, "Attempts"
        ] = 2  # This assumes rp with 1 pitch and no prior ticks had one prior attempt
    else:
...
@st.experimental_memo
def clean_send_plots(
    df_source, selected_rgrade_array, r_grade_fil, selected_bgrade_array, b_grade_fil
):
    ...
    df_usendf_r.loc[
        df_usendf_r["Lead Style"].isin(CLEAN_SEND_FIRST), "Attempts"
    ] = ""  # Optionally change attempts from blank to 1 or other
    ...
    df_usendf_b.loc[
        df_usendf_b["Lead Style"].isin(CLEAN_SEND_FIRST), "Attempts"
    ] = ""  # Optionally change attempts from blank to 1 or other
    ...
    rpy_fig = px.bar(
        ...
        y="Rating",
        orientation="h",
        height=45 * len(r_grade_fil),
        category_orders={"Rating": selected_rgrade_array[::-1]},
        text="Attempts",
        custom_data=["Route", "Date Formatted", "Location", "Length", "Avg Stars"],
    )
    rpy_fig.update_layout(
        font={"family": "Courier New", "color": "black", "size": 20},
        title={"text": "<b>Climbing Pyramid</b>", "x": 0.5, "font_size": 30},
        xaxis={"title": "Number of Routes Sent"},
        yaxis={"title": "", "type": "category"},
        paper_bgcolor="#ece5dc",
        plot_bgcolor="#F5D3A5",
        bargap=0,
    )
    rpy_fig.update_traces(
        marker_color="#ac7c5c",
        marker_line_width=2,
        marker_line_color="#8b532d",
        textposition="inside",
    ...
    rhistory_fig.update_layout(
        font={"family": "Courier New", "color": "black", "size": 20},
        title={"text": "<b>Send by Date</b>", "x": 0.5, "font_size": 30},
        xaxis={"title": "Date"},
        yaxis={"title": "", "type": "category"},
        paper_bgcolor="#ece5dc",
        plot_bgcolor="#F5D3A5",
    ...
    bpyr_fig = px.bar(
        ...
        y="Rating",
        orientation="h",
        height=45 * len(b_grade_fil),
        category_orders={"Rating": selected_bgrade_array[::-1]},
        text="Attempts",
        custom_data=["Route", "Date Formatted", "Location", "Length", "Avg Stars"],
    )
    bpyr_fig.update_layout(
        font={"family": "Courier New", "color": "black", "size": 18},
        title={"text": "<b>Climbing Pyramid</b>", "x": 0.5, "font_size": 30},
        xaxis={"title": "Number of Problems Sent"},
        yaxis={"title": "", "type": "category"},
        paper_bgcolor="#ece5dc",
        plot_bgcolor="#F5D3A5",
        bargap=0,
    )
    bpyr_fig.update_traces(
        marker_color="#ac7c5c",
        marker_line_width=2,
        marker_line_color="#8b532d",
        textposition="inside",
...
def tick_report(df_source):
    ...
    df_bold_leads = df_source[
        (df_source["Lead Ratio"] < 0.4)
        & (df_source["Style"] == "Lead")
        & (df_source["Route Type"] != "Boulder")
    ].sort_values(by="Lead Ratio", ascending=True)
    # User onsighted something rarely onsighted
    df_impressive_OS = df_source[
        (df_source["OS Ratio"] < 0.35)
        & (df_source["Flash/Onsight"] == True)
        & (df_source["Route Type"] != "Boulder")
    ].sort_values(by="OS Ratio")
    # User fell on something rarely fallen on
    df_woops_falls = df_source[
        (df_source["OS Ratio"] > 0.8)
        & (df_source["Style"] == "Lead")
        & (df_source["Lead Style"] == "Fell/Hung")
        & (df_source["Route Type"] != "Boulder")
    ].sort_values(by="OS Ratio", ascending=True)
