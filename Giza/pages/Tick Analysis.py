...
if ~st.session_state.df_usend_uniq_ticks.empty:
    data_source_type_selections.insert(0, 'Session Dataset')
data_source_type = col1.radio("Data Source Selection", data_source_type_selections, help="placeholder ", horizontal=True)
if data_source_type == 'Session Dataset':
    col1.info(f"There is a scraped dataset saved on this session under user reference: {st.session_state.upload_link_ref}", icon="ℹ️")
...
with col3:
    ...
    st.session_state.df_usend_uniq_ticks = grade_homo(st.session_state.df_usend_uniq_ticks, *grade_settings)
